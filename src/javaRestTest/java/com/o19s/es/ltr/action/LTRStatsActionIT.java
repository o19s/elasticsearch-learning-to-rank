...
public class LTRStatsActionIT extends BaseIntegrationTest {
  ...
  public void testAllStatsDefaultEmptyStore() throws ExecutionException, InterruptedException {
    ...
    assertTrue(
        featureCacheStats.containsKey(CacheStatsOnNodeSupplier.Stat.CACHE_HIT_COUNT.getName()));
    assertTrue(
        featureCacheStats.containsKey(CacheStatsOnNodeSupplier.Stat.CACHE_MISS_COUNT.getName()));
    assertTrue(
        featureCacheStats.containsKey(
            CacheStatsOnNodeSupplier.Stat.CACHE_EVICTION_COUNT.getName()));
    assertTrue(
        featureCacheStats.containsKey(CacheStatsOnNodeSupplier.Stat.CACHE_ENTRY_COUNT.getName()));
    assertTrue(
        featureCacheStats.containsKey(
            CacheStatsOnNodeSupplier.Stat.CACHE_MEMORY_USAGE_IN_BYTES.getName()));
  }
  ...
  public void testStoreStats() throws Exception {
    ...
    assertStoreStats(
        (Map<String, Object>) stores.get(DEFAULT_STORE_NAME), infos.get(DEFAULT_STORE_NAME));
  ...
  private LTRStatsNodesResponse executeRequest() throws ExecutionException, InterruptedException {
    ...
    Set<String> statsToBeRetrieved =
        new HashSet<>(
            Arrays.asList(
                StatName.PLUGIN_STATUS.getName(),
                StatName.CACHE.getName(),
