...
public class CacheStatsOnNodeSupplierTests extends ESTestCase {
  ...
  private void assertCacheStats(
      Map<String, Object> stat, long hits, long misses, long evictions, int entries) {
  ...
  private void assertMemoryUsage(Map<String, Map<String, Object>> stats) {
    assertTrue(
        (long)
                stats
                    .get(Stat.CACHE_FEATURE.getName())
                    .get(Stat.CACHE_MEMORY_USAGE_IN_BYTES.getName())
            > 0);
    assertTrue(
        (long)
                stats
                    .get(Stat.CACHE_FEATURE_SET.getName())
                    .get(Stat.CACHE_MEMORY_USAGE_IN_BYTES.getName())
            > 0);
    assertTrue(
        (long) stats.get(Stat.CACHE_MODEL.getName()).get(Stat.CACHE_MEMORY_USAGE_IN_BYTES.getName())
            > 0);
  }
}
