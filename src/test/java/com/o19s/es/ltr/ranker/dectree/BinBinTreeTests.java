...
public class BinBinTreeTests extends LuceneTestCase {
    public void testScore() throws Exception {
        ...
        NaiveAdditiveDecisionTree naive = NaiveAdditiveDecisionTreeTests.generateRandomDecTree(1, 1000,
                1, 1000,
                1, 8, counts);
        BinBinTree binTree = naive.toBinBinTree();
        assertEquals(naive.size(), binTree.size());
        assertTrue(naive.ramBytesUsed() > binTree.ramBytesUsed());

        int nPass = TestUtil.nextInt(random(), 10, 8916);
        LtrTestUtils.assertRankersHaveSameScores(naive, binTree, nPass);
    }

    public void testSimplScore() throws IOException {
        BinBinTree ranker = NaiveAdditiveDecisionTreeTests.parseTreeModel("simple_tree.txt").toBinBinTree();
