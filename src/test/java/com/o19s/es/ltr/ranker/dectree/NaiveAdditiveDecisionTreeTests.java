...
public class NaiveAdditiveDecisionTreeTests extends LuceneTestCase {
  ...
  public void testName() {
    NaiveAdditiveDecisionTree dectree =
        new NaiveAdditiveDecisionTree(
  ...
  public void testScore() throws IOException {
    NaiveAdditiveDecisionTree ranker =
        parseTreeModel("simple_tree.txt", Normalizers.get(Normalizers.NOOP_NORMALIZER_NAME));
  ...
  public void testSigmoidScore() throws IOException {
    NaiveAdditiveDecisionTree ranker =
        parseTreeModel("simple_tree.txt", Normalizers.get(Normalizers.SIGMOID_NORMALIZER_NAME));
    LtrRanker.FeatureVector vector = ranker.newFeatureVector(null);
    vector.setFeatureScore(0, 1);
    vector.setFeatureScore(1, 2);
    vector.setFeatureScore(2, 3);
  ...
  public void testPerfAndRobustness() {
    SimpleCountRandomTreeGeneratorStatsCollector counts =
        new SimpleCountRandomTreeGeneratorStatsCollector();
    ...
    LOG.info(
        "Scored {} docs with {} trees/{} features within {}ms ({} ms/doc), "
            + "{} nodes ({} splits & {} leaves) ",
        nPass,
        counts.trees.get(),
        ranker.size(),
    ...
    long expectedApprox =
        counts.splits.get()
            * (NUM_BYTES_OBJECT_HEADER
                ...
                + Integer.BYTES * 3);
    expectedApprox +=
        counts.leaves.get() * (NUM_BYTES_ARRAY_HEADER + NUM_BYTES_OBJECT_HEADER + Float.BYTES);
  ...
  public static NaiveAdditiveDecisionTree generateRandomDecTree(
    ...
    return new NaiveAdditiveDecisionTree(
        trees, weights, nbFeatures, Normalizers.get(Normalizers.NOOP_NORMALIZER_NAME));
  }
  ...
  private NaiveAdditiveDecisionTree parseTreeModel(String textRes, Normalizer normalizer)
      throws IOException {
    ...
    NaiveAdditiveDecisionTree.Node[] trees =
        new NaiveAdditiveDecisionTree.Node[treesAndWeight.size()];
    ...
    NaiveAdditiveDecisionTree.Node parseTree() {
      ...
      if (line.contains("- output")) {
        ...
        return new NaiveAdditiveDecisionTree.Split(
            left, right, ord, threshold, leftNodeId, rightNodeId, missingNodeId);
      } else {
        throw new IllegalArgumentException("Invalid tree");
      }
    }
  ...
  public static class RandomTreeGenerator {
    ...
    public RandomTreeGenerator(
        int maxFeat, int minDepth, int maxDepth, RandomTreeGeneratorStatsCollector collector) {
    ...
    private NaiveAdditiveDecisionTree.Node newSplit(int depth) {
      ...
      return new NaiveAdditiveDecisionTree.Split(
          newNode(depth), newNode(depth), feature, thresh, 1, 2, 1);
    }
  ...
  public static class SimpleCountRandomTreeGeneratorStatsCollector
      implements RandomTreeGeneratorStatsCollector {
