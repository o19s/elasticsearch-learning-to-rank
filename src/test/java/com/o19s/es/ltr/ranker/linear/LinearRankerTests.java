...
public class LinearRankerTests extends LuceneTestCase {
  ...
  public void testRamSize() {
    ...
    int expectedSize =
        ranker.size() * Float.BYTES + NUM_BYTES_ARRAY_HEADER + NUM_BYTES_OBJECT_HEADER;
    assertThat(
        ranker.ramBytesUsed(),
        allOf(greaterThan((long) (expectedSize * 0.66F)), lessThan((long) (expectedSize * 1.33F))));
  }
  ...
  public void testPerfAndRobustness() {
    ...
    LOG.info(
        "Scored {} docs with {} features within {}ms ({} ms/doc)",
        nPass,
        ranker.size(),
        time,
