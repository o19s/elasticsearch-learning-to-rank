...
public class LoggingFetchSubPhaseTests extends LuceneTestCase {
  ...
  public static void init() throws Exception {
    ...
    try (IndexWriter writer =
        new IndexWriter(directory, newIndexWriterConfig(new StandardAnalyzer()))) {
  ...
  public void testLogging() throws IOException {
    ...
    LoggingFetchSubPhase.HitLogConsumer logger1 =
        new LoggingFetchSubPhase.HitLogConsumer("logger1", query1.featureSet(), true);
    LoggingFetchSubPhase.HitLogConsumer logger2 =
        new LoggingFetchSubPhase.HitLogConsumer("logger2", query2.featureSet(), false);
    ...
    LoggingFetchSubPhaseProcessor processor =
        new LoggingFetchSubPhaseProcessor(() -> new Tuple<>(weight, loggers));
    ...
    for (SearchHit hit : hits) {
      ...
      assertEquals(
          (float) expectedScore, (Float) log1.get(1).get("value"), Math.ulp((float) expectedScore));
      assertEquals(
          (float) expectedScore, (Float) log1.get(1).get("value"), Math.ulp((float) expectedScore));
    }
  }
        ...
        new SimpleCollector() {
          ...
          public void collect(int doc) throws IOException {
            if (hits.size() < minHits || (random().nextBoolean() && hits.size() < maxHits)) {
              ...
              String id = d.get("id");
              SearchHit hit = new SearchHit(doc, id);
              Source source = null;
