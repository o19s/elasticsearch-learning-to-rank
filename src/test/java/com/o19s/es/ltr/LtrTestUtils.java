...
public class LtrTestUtils {
  ...
  public static StoredLtrModel randomLinearModel(String name, StoredFeatureSet set)
      throws IOException {
    ...
    builder.endObject();
    return new StoredLtrModel(
        name, set, LinearRankerParser.TYPE, Strings.toString(builder), false, randomFtrNorms(set));
  }
  ...
  public static StoredFeatureNormalizers randomFtrNorms(FeatureSet set) {
    ...
    for (int i = 0; i < set.size(); i++) {
      ...
      if (random().nextBoolean()) {
        ...
        if (random().nextBoolean()) {
          StandardFeatureNormDefinition stdFtrNormDefn =
              new StandardFeatureNormDefinition(ftr.name());
          stdFtrNormDefn.setMean(random().nextFloat());
          stdFtrNormDefn.setStdDeviation(random().nextFloat());
          ftrNormDefns.add(stdFtrNormDefn);

        } else {
          MinMaxFeatureNormDefinition minMaxFtrNormDefn =
              new MinMaxFeatureNormDefinition(ftr.name());
  ...
  public static LtrRanker buildRandomRanker(int fSize) {
    ...
    if (random().nextBoolean()) {
      ...
      ranker =
          NaiveAdditiveDecisionTreeTests.generateRandomDecTree(
              fSize, TestUtil.nextInt(random(), 1, 50), 5, 50, null);
    }
    return ranker;
  }
  ...
  public static <R, E extends Exception> IntFunction<R> wrapIntFuncion(
      CheckedFunction<Integer, R, E> f) {
    return (p) -> {
      try {
        return f.apply(p);
      } catch (Exception e) {
