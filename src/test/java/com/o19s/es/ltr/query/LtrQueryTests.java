...
public class LtrQueryTests extends LuceneTestCase {
  ...
  private int[] range(int start, int stop) {
    ...
    for (int i = 0; i < stop - start; i++) result[i] = start + i;
  ...
  private final String[] docs =
      new String[] {
        "how now brown cow", "brown is the color of cows", "brown cow", "banana cows are yummy"
      };
  ...
  public void setupIndex() throws IOException {
    ...
    indexWriterUnderTest =
        new RandomIndexWriter(
            random(), dirUnderTest, newIndexWriterConfig().setSimilarity(similarity));
  ...
  public Map<String, Map<Integer, Float>> getFeatureScores(
      List<PrebuiltFeature> features, final float missingScore) throws IOException {
    ...
    LogLtrRanker.LogConsumer logger =
        new LogLtrRanker.LogConsumer() {
          @Override
  ...
  public void checkModelWithFeatures(
      List<PrebuiltFeature> features, int[] modelFeatures, Map<Integer, Normalizer> ftrNorms)
      throws IOException {
    ...
    Ranker ranker =
        trainer.train(
            /*what type of model ot train*/ RANKER_TYPE.RANKNET,
            /*The training data*/ samples
            /*which features to use*/ ,
            featuresToUse
            /*how to score ranking*/ ,
            new NDCGScorer());
    float[] scores = {
  ...
  private void assertScoresMatch(
      List<PrebuiltFeature> features, float[] scores, RankerQuery ltrQuery, ScoreDoc scoreDoc)
      throws IOException {
  ...
  private RankerQuery toRankerQuery(
      List<PrebuiltFeature> features, Ranker ranker, Map<Integer, Normalizer> ftrNorms) {
    LtrRanker ltrRanker = new RanklibRanker(ranker, features.size());
    if (ftrNorms.size() > 0) {
      ltrRanker = new FeatureNormalizingRanker(ltrRanker, ftrNorms);
    }
    PrebuiltLtrModel model =
        new PrebuiltLtrModel(ltrRanker.name(), ltrRanker, new PrebuiltFeatureSet(null, features));
  ...
  public void testExplainWithNames() throws IOException {
    //     public LambdaMART(List<RankList> samples, int[] features, MetricScorer scorer) {
    String userQuery = "brown cow";
    List<PrebuiltFeature> features =
        Arrays.asList(
            new PrebuiltFeature(
                "funky_term_q", new TermQuery(new Term("field", userQuery.split(" ")[0]))),
  ...
  public void testOnRewrittenQueries() throws IOException {
    ...
    BlendedTermQuery.Builder bldr =
        new BlendedTermQuery.Builder().setRewriteMethod(BlendedTermQuery.DISJUNCTION_MAX_REWRITE);
    for (Term t : termsToBlend) {
      bldr.add(t);
    }
    Query blended = bldr.build();
    List<Query> features =
        Arrays.asList(new TermQuery(new Term("field", userQuery.split(" ")[0])), blended);
