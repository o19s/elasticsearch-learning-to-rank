...
public class ValidatingLtrQueryBuilderTests
    extends AbstractQueryTestCase<ValidatingLtrQueryBuilder> {
  ...
  protected ValidatingLtrQueryBuilder doCreateTestQueryBuilder() {
    ...
    Function<String, StoredFeature> buildFeature =
        (n) ->
            new StoredFeature(
                n,
    ...
    BiFunction<Integer, String, StoredFeatureSet> buildFeatureSet =
        (i, name) ->
            new StoredFeatureSet(
                name,
                IntStream.range(0, i)
                    .mapToObj((idx) -> buildFeature.apply("feature" + idx))
                    .collect(Collectors.toList()));
    Function<String, StoredLtrModel> buildModel =
        (name) ->
            new StoredLtrModel(
                name,
  ...
  protected void doAssertLuceneQuery(
      ValidatingLtrQueryBuilder queryBuilder, Query query, SearchExecutionContext context)
      throws IOException {
    if (StoredFeature.TYPE.equals(queryBuilder.getElement().type())) {
      ...
      RankerQuery q = (RankerQuery) query;
      assertEquals(
          ((StoredLtrModel) queryBuilder.getElement()).featureSet().name(), q.featureSet().name());
    } else {
      throw new AssertionError(
          "Invalid storable element type : " + queryBuilder.getElement().type());
    }
  }
}
