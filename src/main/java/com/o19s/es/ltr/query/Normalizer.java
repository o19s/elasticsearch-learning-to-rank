...
public interface Normalizer extends NamedWriteable, ToXContent {
    ...
    Explanation explain(float sourceScore, Explanation sourceExplanation);
    ...
    class IntervalNormalizer implements Normalizer {
        ...
        static final ConstructingObjectParser<IntervalNormalizer, Void> PARSER = new ConstructingObjectParser<>(NAME.getPreferredName(),
                (o) -> new IntervalNormalizer((float) o[0], (float) o[1],
                        o[3] != null ? (boolean) o[3] : false, (UnitIntervalNormalizer) o[2]));

        static {
            PARSER.declareFloat(ConstructingObjectParser.constructorArg(), FROM);
            PARSER.declareFloat(ConstructingObjectParser.constructorArg(), TO);
        ...
        public IntervalNormalizer(float from, float to, boolean inclusive, UnitIntervalNormalizer normalizer) {
            this.from = from;
            this.to = to;
        ...
        IntervalNormalizer(StreamInput in) throws IOException {
            this(in.readFloat(), in.readFloat(), in.readBoolean(), in.readNamedWriteable(UnitIntervalNormalizer.class));
        }
        ...
        @Override
        public Explanation explain(float sourceScore, Explanation sourceExplanation) {
            return Explanation.match(normalize(sourceScore),
    ...
    class MinMax implements UnitIntervalNormalizer {
        ...
        static final ConstructingObjectParser<MinMax,Void> PARSER = new ConstructingObjectParser<>(NAME.getPreferredName(),
                (o) -> new MinMax((float) o[0], (float) o[1]));
        static {
            PARSER.declareFloat(ConstructingObjectParser.constructorArg(), MIN);
            PARSER.declareFloat(ConstructingObjectParser.constructorArg(), MAX);
        }
        ...
        public boolean equals(Object o) {
            ...
            return Float.compare(minMax.min, min) == 0 &&
                    Float.compare(minMax.max, max) == 0;
        }
    ...
    class Saturation implements UnitIntervalNormalizer {
        ...
        static final ConstructingObjectParser<Saturation,Void> PARSER = new ConstructingObjectParser<>(NAME.getPreferredName(),
                (o) -> new Saturation((float) o[0], (float) o[1]));
        static {
            PARSER.declareFloat(ConstructingObjectParser.constructorArg(), K);
            PARSER.declareFloat(ConstructingObjectParser.constructorArg(), A);
        }

        public Saturation(float k, float a) {
            if (k <= 0) {
        ...
        public XContentBuilder doToXContent(XContentBuilder builder, Params params) throws IOException {
            return builder.startObject()
                    .field(K.getPreferredName(), (float) k)
                    .field(A.getPreferredName(), (float) a)
                    .endObject();
        }
    ...
    class Logistic implements UnitIntervalNormalizer {
        ...
        static final ConstructingObjectParser<Logistic,Void> PARSER = new ConstructingObjectParser<>(NAME.getPreferredName(),
                (o) -> new Logistic((float) o[0], (float) o[1]));

        static {
            PARSER.declareFloat(ConstructingObjectParser.constructorArg(), K);
            PARSER.declareFloat(ConstructingObjectParser.constructorArg(), X0);
        }

        public Logistic(float k, float x0) {
            this.k = k;
            this.x0 = x0;
        }
        ...
        @Override
        public float normalize(float score) {
            return (float) (1D / (1D + Math.exp(-k * ((score - x0)))));
        }
        ...
        public XContentBuilder doToXContent(XContentBuilder builder, Params params) throws IOException {
            return builder.startObject()
                    .field(K.getPreferredName(), (float) k)
                    .field(X0.getPreferredName(), (float) x0)
                    .endObject();
        }
    ...
    class NoopNormalizer implements Normalizer {
        ...
        public Explanation explain(float sourceScore, Explanation sourceExplanation) {
            return Explanation.match(normalize(sourceScore),"noop normalizer",
                    sourceExplanation);
        }
