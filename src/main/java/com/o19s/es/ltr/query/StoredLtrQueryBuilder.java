...
public class StoredLtrQueryBuilder extends AbstractQueryBuilder<StoredLtrQueryBuilder>
  ...
  static {
    ...
    PARSER.declareField(
        StoredLtrQueryBuilder::params, XContentParser::map, PARAMS, ObjectParser.ValueType.OBJECT);
    PARSER.declareStringArray(StoredLtrQueryBuilder::activeFeatures, ACTIVE_FEATURES);
    declareStandardFields(PARSER);
  }

  /**
   * Injected context used to load a {@link FeatureStore} when running {@link
   * #doToQuery(SearchExecutionContext)}
   */
  ...
  public StoredLtrQueryBuilder(FeatureStoreLoader storeLoader, StreamInput input)
      throws IOException {
    super(input);
  ...
  public static StoredLtrQueryBuilder fromXContent(
      FeatureStoreLoader storeLoader, XContentParser parser) throws IOException {
  ...
  protected RankerQuery doToQuery(SearchExecutionContext context) throws IOException {
    ...
    LtrQueryContext ltrQueryContext =
        new LtrQueryContext(
            context,
  ...
  protected int doHashCode() {
    return Objects.hash(
        modelName, featureScoreCacheFlag, featureSetName, storeName, params, activeFeatures);
  }
