...
/**
 * Lucene query designed to apply a ranking model provided by {@link LtrRanker} This query is not
 * designed for retrieval, in other words it will score all the docs in the index and thus must be
 * used either in a rescore phase or within a BooleanQuery and an appropriate filter clause.
 */
public class RankerQuery extends Query {
  ...
  /**
   * Build a RankerQuery based on a prebuilt model. Prebuilt models are not parametrized as they
   * contain only {@link com.o19s.es.ltr.feature.PrebuiltFeature}
   *
  ...
  public RankerQuery toLoggerQuery(LogLtrRanker.LogConsumer consumer) {
    ...
    return new RankerQuery(
        queries, features, new LogLtrRanker(newRanker, consumer), featureScoreCache);
  }
  ...
  public Query rewrite(IndexReader reader) throws IOException {
    ...
    return rewritten
        ? new RankerQuery(rewrittenQueries, features, ranker, featureScoreCache)
        : this;
  }
  ...
  public static class RankerWeight extends Weight {
    ...
    public Explanation explain(LeafReaderContext context, int doc) throws IOException {
      ...
      return Explanation.match(
          modelScore, " LtrModel: " + ranker.name() + " using features:", subs);
    }
    ...
    public RankerScorer scorer(LeafReaderContext context) throws IOException {
      ...
      DisjunctionDISI rankerIterator =
          new DisjunctionDISI(
    ...
    class RankerScorer extends Scorer {
      /**
       * NOTE: Switch to ChildScorer and {@link #getChildren()} if it appears to be useful for
       * logging
       */
      ...
      /**
       * Return the maximum score that documents between the last {@code target} that this iterator
       * was {@link #advanceShallow(int) shallow-advanced} to included and {@code upTo} included.
       */
      @Override
  ...
  /**
   * Driven by a main iterator and tries to maintain a list of sub iterators Mostly needed to avoid
   * calling {@link Scorer#iterator()} to directly advance from {@link
   * RankerWeight.RankerScorer#score()} as some Scorer implementations will instantiate new objects
