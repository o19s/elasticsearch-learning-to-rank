...
public class DerivedExpressionQuery extends Query implements LtrRewritableQuery {
  ...
  public DerivedExpressionQuery(
      FeatureSet features, DoubleValuesScript expr, Map<String, Double> queryParamValues) {
  ...
  static final class FVDerivedExpressionQuery extends Query {
    ...
    FVDerivedExpressionQuery(
        DerivedExpressionQuery query, Supplier<LtrRanker.FeatureVector> fvSupplier) {
      this.query = query;
  ...
  static class FVWeight extends Weight {
    ...
    public Scorer scorer(LeafReaderContext context) throws IOException {
      ...
      DoubleValuesSource src =
          expression.getDoubleValuesSource(
              (name) -> {
