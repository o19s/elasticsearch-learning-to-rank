...
public class TransportLTRStatsAction
    extends TransportNodesAction<
        LTRStatsNodesRequest, LTRStatsNodesResponse, LTRStatsNodeRequest, LTRStatsNodeResponse> {
  ...
  protected LTRStatsNodesResponse newResponse(
    ...
    Map<String, Object> clusterStats =
        ltrStats.getClusterStats().entrySet().stream()
            .filter(e -> statsToBeRetrieved.contains(e.getKey()))
            .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getStatValue()));

    return new LTRStatsNodesResponse(
        clusterService.getClusterName(), nodeResponses, failures, clusterStats);
  }
  ...
  @Override
  protected LTRStatsNodeResponse newNodeResponse(StreamInput in, DiscoveryNode node)
      throws IOException {
  ...
  protected LTRStatsNodeResponse nodeOperation(LTRStatsNodeRequest request, Task task) {
    ...
    Map<String, Object> statValues =
        ltrStats.getNodeStats().entrySet().stream()
            .filter(e -> statsToBeRetrieved.contains(e.getKey()))
            .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getStatValue()));
