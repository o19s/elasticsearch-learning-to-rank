...
public class TransportCreateModelFromSetAction
    extends HandledTransportAction<CreateModelFromSetRequest, CreateModelFromSetResponse> {
  ...
  public TransportCreateModelFromSetAction(
    ...
    super(
        CreateModelFromSetAction.NAME,
        transportService,
        actionFilters,
        CreateModelFromSetRequest::new,
  ...
  protected void doExecute(
    ...
    GetRequest getRequest =
        new GetRequest(request.getStore())
    ...
    getAction.execute(
        task,
        getRequest,
        ActionListener.wrap((r) -> this.doStore(task, r, request, listener), listener::onFailure));
  }
  ...
  private void doStore(
    ...
    if (!response.isExists()) {
      throw new IllegalArgumentException(
          "Stored feature set [" + request.getFeatureSetName() + "] does not exist");
    }
    if (request.getExpectedSetVersion() != null
        && request.getExpectedSetVersion() != response.getVersion()) {
      throw new IllegalArgumentException(
          "Stored feature set ["
    ...
    try {
      set =
          IndexFeatureStore.parse(
              StoredFeatureSet.class, StoredFeatureSet.TYPE, response.getSourceAsBytesRef());
    } catch (IOException ioe) {
      throw new IllegalStateException(
          "Cannot parse stored feature set [" + request.getFeatureSetName() + "]", ioe);
    }
    ...
    FeatureStoreRequest featureStoreRequest =
        new FeatureStoreRequest(request.getStore(), model, FeatureStoreRequest.Action.CREATE);
