...
public class TransportFeatureStoreAction
    extends HandledTransportAction<FeatureStoreRequest, FeatureStoreResponse> {
  ...
  public TransportFeatureStoreAction(
    ...
    super(
        FeatureStoreAction.NAME,
        false,
        transportService,
        actionFilters,
        FeatureStoreRequest::new,
  ...
  protected void doExecute(
      Task task, FeatureStoreRequest request, ActionListener<FeatureStoreResponse> listener) {
  ...
  private Optional<ClearCachesNodesRequest> buildClearCache(FeatureStoreRequest request)
    ...
    if (request.getAction() == FeatureStoreRequest.Action.UPDATE) {
      ClearCachesAction.ClearCachesNodesRequest clearCachesNodesRequest =
          new ClearCachesAction.ClearCachesNodesRequest();
      switch (request.getStorableElement().type()) {
        case StoredFeature.TYPE:
          clearCachesNodesRequest.clearFeature(
              request.getStore(), request.getStorableElement().name());
          return Optional.of(clearCachesNodesRequest);
        case StoredFeatureSet.TYPE:
          clearCachesNodesRequest.clearFeatureSet(
              request.getStore(), request.getStorableElement().name());
          return Optional.of(clearCachesNodesRequest);
      }
    }
  ...
  private IndexRequest buildIndexRequest(Task parentTask, FeatureStoreRequest request)
      throws IOException {
        ...
        wrap(
            (r) -> {
              if (r.getFailedShards() > 0) {
                ...
                throw new IllegalArgumentException(
                    "Validating the element caused "
                        + r.getFailedShards()
                        + " shard failures, see root cause: "
                        + failure.reason(),
                    failure.getCause());
              }
              onSuccess.run();
            ...
            (e) ->
                listener.onFailure(
                    new IllegalArgumentException("Cannot store element, validation failed.", e))));
  }

  /**
   * Prepare a Runnable to send an index request to store the element, invalidates the cache on
   * success
   */
  private void store(
      FeatureStoreRequest request, Task task, ActionListener<FeatureStoreResponse> listener) {
      ...
      client.execute(
          ...
          wrap(
              (r) -> {
                ...
                clearCachesNodesRequest.ifPresent(
                    (req) ->
                        clearCachesAction.execute(
                            task,
                            req,
                            wrap((r2) -> {}, (e) -> logger.error("Failed to clear cache", e))));
