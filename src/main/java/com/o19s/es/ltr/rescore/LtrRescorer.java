...
public class LtrRescorer implements Rescorer {
    ...
    TopDocs bulkRescore(TopDocs topDocs, IndexSearcher searcher, LtrRescoreContext ctx) throws IOException {
        ...
        for (int i = wSize; i < docs.length; i++) {
            ...
            original.score = ctx.normalizedQueryScore(original.score);
        }
    ...
    TopDocs classicRescore(TopDocs topDocs, IndexSearcher searcher, LtrRescoreContext ctx) throws IOException {
        ...
        QueryRescorer rescorer = new QueryRescorer(ctx.query) {
            ...
            protected float combine(float firstPassScore, boolean secondPassMatches, float secondPassScore) {
                ...
                return ctx.normalizedQueryScore(firstPassScore);
            }
        };
        ...
        for (int i = topDocsRescored.scoreDocs.length; i < topDocs.scoreDocs.length; i++) {
            ...
            doc.score = ctx.queryNormalizer.normalize(doc.score)*ctx.queryWeight;
        }
    ...
    float combine(float queryScore, float rescoreQueryScore, LtrRescoreContext ctx) {
        float weightedRescoreScore = ctx.rescoreQueryNormalizer.normalize(rescoreQueryScore)*ctx.rescoreQueryWeight;
        if (ctx.scoreMode == LtrRescoreMode.Replace) {
            return weightedRescoreScore;
        }
        float weightedQueryScore = ctx.queryNormalizer.normalize(queryScore)*ctx.queryWeight;
        return ctx.scoreMode.combine(weightedQueryScore, weightedRescoreScore);
    }
    ...
    private Explanation explainCombine(Explanation queryExplanation, Explanation rescoreQueryExplanation, LtrRescoreContext ctx) {
        ...
        queryExplain = Explanation.match(queryExplain.getValue() * ctx.queryWeight, "product of:",
                queryExplain, Explanation.match(ctx.rescoreQueryWeight, "primaryWeight"));
        ...
        if (rescoreQueryExplanation.isMatch()) {
            ...
            rescoreExplain = Explanation.match(rescoreExplain.getValue() * ctx.rescoreQueryWeight, "product of:",
                    rescoreExplain, Explanation.match(ctx.rescoreQueryWeight, "secondaryWeight"));
    ...
    public static class LtrRescoreContext extends RescoreContext {
        ...
        public float normalizedQueryScore(float score) {
            return queryNormalizer.normalize(score)* queryWeight;
        }

        public float normalizedRescoreQueryScore(float score) {
        ...
        public LtrRescoreContext setQueryWeight(float queryWeight) {
        ...
        public LtrRescoreContext setRescoreQueryWeight(float rescoreQueryWeight) {
