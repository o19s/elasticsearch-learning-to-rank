...
public class PrecompiledExpressionFeature implements Feature, Accountable {
  ...
  private static final long BASE_RAM_USED =
      RamUsageEstimator.shallowSizeOfInstance(PrecompiledExpressionFeature.class);
  ...
  private PrecompiledExpressionFeature(
      String name, DoubleValuesScript expression, Collection<String> queryParams) {
    this.name = name;
  ...
  /**
   * If a param is an expression variable then we need to use it as {@link Expression#variables} in
   * lucene expression exposed via Bindings
   *
      ...
      if (expressionVariables.contains(param)) {
        try {
          ...
          throw new IllegalArgumentException(
              "parameter: " + param + " expected to be of type Double");
        }
      }
    }
      ...
      if (!set.hasFeature(var) && !queryParams.contains(var)) {
        throw new IllegalArgumentException(
            "Derived feature ["
                + this.name
                + "] refers "
                + "to unknown feature or parameter: ["
                + var
                + "]");
      }
      if (set.hasFeature(var) && queryParams.contains(var)) {
