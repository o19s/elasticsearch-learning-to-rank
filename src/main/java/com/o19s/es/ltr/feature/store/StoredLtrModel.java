...
public class StoredLtrModel implements StorableElement {
  ...
  static {
    ...
    PARSER.declareObject(
        ParsingState::setFeatureSet, (parser, ctx) -> StoredFeatureSet.parse(parser), FEATURE_SET);
  ...
  public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
    ...
    if (modelAsString) {
      ...
      try (XContentParser parser =
          JsonXContent.jsonXContent.createParser(XContentParserConfiguration.EMPTY, rankingModel)) {
  ...
  public static class LtrModelDefinition implements Writeable {
    ...
    static {
      ...
      PARSER.declareField(
          (p, d, c) -> d.parseModel(p),
          MODEL_DEFINITION,
          ObjectParser.ValueType.OBJECT_ARRAY_OR_STRING);

      PARSER.declareNamedObjects(
          LtrModelDefinition::setNamedFeatureNormalizers,
    ...
    public static LtrModelDefinition parse(XContentParser parser, Void ctx) throws IOException {
      ...
      if (def.definition == null) {
        throw new ParsingException(
            parser.getTokenLocation(), "Field [model.definition] is mandatory");
      }
      return def;
    }
