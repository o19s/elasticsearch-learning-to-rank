...
public class ScriptFeature implements Feature {
  ...
  public static ScriptFeature compile(StoredFeature feature) {
    try {
      XContentParser xContentParser =
          XContentType.JSON
              .xContent()
              .createParser(XContentParserConfiguration.EMPTY, feature.template());

      return new ScriptFeature(
          feature.name(), Script.parse(xContentParser, "native"), feature.queryParams());
    } catch (IOException e) {
  ...
  public Query doToQuery(
      LtrQueryContext context, FeatureSet featureSet, Map<String, Object> params) {
    ...
    if (baseScriptParams.containsKey("term_stat")) {
      HashMap<String, Object> termspec =
          (HashMap<String, Object>) baseScriptParams.get("term_stat");
    ...
    Script script =
        new Script(
            ...
            nparams);
    ScoreScript.Factory factoryFactory =
        context.getSearchExecutionContext().compile(script, ScoreScript.CONTEXT);
    ScoreScript.LeafFactory leafFactory =
        factoryFactory.newFactory(nparams, context.getSearchExecutionContext().lookup());
    ScriptScoreFunction function =
  ...
  static class LtrScript extends Query implements LtrRewritableQuery {
    ...
    public void visit(QueryVisitor visitor) {
      ...
      visitor
          .getSubVisitor(BooleanClause.Occur.SHOULD, this)
          .consumeTerms(this, terms.toArray(new Term[0]));
    }
  }
  ...
  static class LtrScriptWeight extends Weight {
    ...
    public Scorer scorer(LeafReaderContext context) throws IOException {
      ...
      return new Scorer(this) {
        ...
        /**
         * Return the maximum score that documents between the last {@code target} that this
         * iterator was {@link #advanceShallow(int) shallow-advanced} to included and {@code upTo}
         * included.
         */
        @Override
