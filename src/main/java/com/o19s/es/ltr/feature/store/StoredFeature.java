...
public class StoredFeature implements Feature, Accountable, StorableElement {
          ...
          if (parser.currentToken() == XContentParser.Token.START_OBJECT) {
            ...
            try (XContentBuilder builder = XContentFactory.contentBuilder(XContentType.JSON)) {
              ...
              throw new ParsingException(
                  parser.getTokenLocation(), "Could not parse inline template", e);
            }
          } else {
            return parser.text();
          }
  ...
  public StoredFeature(StreamInput input) throws IOException {
    ...
    queryParams = input.readCollectionAsImmutableList(StreamInput::readString);
  ...
  public StoredFeature(
      String name, List<String> params, String templateLanguage, XContentBuilder template) {
  ...
  public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
    ...
    if (templateAsString) {
      ...
      // it's ok to use NamedXContentRegistry.EMPTY because we don't really parse we copy the
      // structure...
      XContentParser parser =
          XContentFactory.xContent(template)
              .createParser(XContentParserConfiguration.EMPTY, template);
  ...
  public Query doToQuery(LtrQueryContext context, FeatureSet set, Map<String, Object> params) {
    List<String> missingParams =
        queryParams.stream().filter((x) -> !params.containsKey(x)).collect(Collectors.toList());
    ...
    ScriptQueryBuilder builder =
        new ScriptQueryBuilder(new Script(ScriptType.INLINE, templateLanguage, template, params));
    try {
  ...
  private XContentParser createParser(Object source, XContentParserConfiguration config)
      throws IOException {
    if (source instanceof String) {
      ...
      return XContentFactory.xContent(ref.bytes, ref.offset, ref.length)
          .createParser(config, ref.bytes, ref.offset, ref.length);
    } else if (source instanceof byte[]) {
      return XContentFactory.xContent((byte[]) source).createParser(config, (byte[]) source);
    } else {
      throw new IllegalArgumentException(
          "Template engine returned an unsupported object type ["
            ...
            .mapToLong(
                x ->
                    (Character.BYTES * x.length())
                        ...
                        + NUM_BYTES_ARRAY_HEADER)
            .sum()
        + (Character.BYTES * templateLanguage.length())
        + NUM_BYTES_ARRAY_HEADER
        + (Character.BYTES * template.length())
        + NUM_BYTES_ARRAY_HEADER;
  }
