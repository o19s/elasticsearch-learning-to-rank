...
public class StoredFeatureSet implements FeatureSet, Accountable, StorableElement {
  ...
  private final long BASE_RAM_USED =
      RamUsageEstimator.shallowSizeOfInstance(StoredFeatureSet.class);
  ...
  static {
    ...
    PARSER.declareObjectArray(
        ParsingState::setFeatures, (p, c) -> StoredFeature.parse(p), FEATURES);
  }
  ...
  public FeatureSet optimize() {
    ...
    if (optimized) {
      return new OptimizedFeatureSet(
          this.name, optimizedFeatures, Collections.unmodifiableMap(featureMap));
    }
    return this;
  }
  ...
  public StoredFeatureSet(StreamInput input) throws IOException {
    this(input.readString(), input.readCollectionAsImmutableList(StoredFeature::new));
  }
  ...
  /**
   * Generates a new StoredFeatureSet by adding extra features. The name is kept, features provided
   * here are added at the end of existing features.
   *
   * @param features new features to append
   * @return a new StoredFeatureSet
   * @throws IllegalArgumentException if the resulting size of the set exceed MAX_FEATURES or if
   *     uniqueness of feature names is not met.
   */
  ...
  /**
   * Generate a new set by merging features with this set, feature with same names are updated new
   * features are appended. Ordinals of existing features are kept.
   *
  ...
  public StoredFeatureSet merge(List<StoredFeature> mergedFeatures) {
    int merged =
        (int) mergedFeatures.stream().map(StoredFeature::name).filter(this::hasFeature).count();

    if (size() + (mergedFeatures.size() - merged) > MAX_FEATURES) {
      throw new IllegalArgumentException("The resulting feature set would be too large");
    }
    List<StoredFeature> newFeatures =
        new ArrayList<>(this.features.size() + mergedFeatures.size() - merged);
    newFeatures.addAll(this.features);
  ...
  public long ramBytesUsed() {
    return BASE_RAM_USED
        + featureMap.size() * NUM_BYTES_OBJECT_REF
