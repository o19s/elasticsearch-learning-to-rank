...
public class StoredFeatureNormalizers {
  ...
  static {
    PARSER =
        (XContentParser p, Void c, String featureName) -> {
          ...
          ObjectParser<FeatureNormConsumer, String> parser =
              new ObjectParser<>("feature_normalizers", FeatureNormConsumer::new);

          parser.declareObject(
              FeatureNormConsumer::setFtrNormDefn, StandardFeatureNormDefinition.PARSER, STANDARD);
          parser.declareObject(
              FeatureNormConsumer::setFtrNormDefn, MinMaxFeatureNormDefinition.PARSER, MIN_MAX);
    ...
    for (Map.Entry<String, FeatureNormDefinition> ftrNormDefEntry : featureNormalizers.entrySet()) {
      ...
      if (!featureSet.hasFeature(featureName)) {
        throw new ElasticsearchException(
            "Feature " + featureName + " not found in feature set " + featureSet.name());
      }
  ...
  public void writeTo(StreamOutput output) throws IOException {
    ...
    for (Map.Entry<String, FeatureNormDefinition> featureNormEntry :
        this.featureNormalizers.entrySet()) {
