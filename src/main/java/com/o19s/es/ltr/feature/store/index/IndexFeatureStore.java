...
public class IndexFeatureStore implements FeatureStore {
  ...
  public static final Setting<Integer> STORE_VERSION_PROP =
      Setting.intSetting(
          "index.ltrstore_version", VERSION, -1, Integer.MAX_VALUE, Setting.Property.IndexScope);
  ...
  /**
   * List of invalid for a feature store name: feature, features, featureSet, featureSets,
   * feature_Set, feature_Sets, featureset, featuresets, feature_set, feature_sets, model, models
   */
  private static final Pattern INVALID_NAMES =
      Pattern.compile("^(features?[*]?|feature_[sS]ets?|models?)$");

  private static final ObjectParser<ParserState, Void> SOURCE_PARSER;

  static {
    SOURCE_PARSER = new ObjectParser<>("", true, ParserState::new);
    SOURCE_PARSER.declareField(
  ...
  public IndexFeatureStore(
      String index, Supplier<Client> clientSupplier, LtrRankerParserFactory factory) {
  ...
  /**
   * Infer the store name based on an elasticsearch index name This function is only meant for user
   * display, _default_ is returned in case indexName equals to DEFAULT_STORE.
   *
  ...
  public static String storeName(String indexName) {
    if (!isIndexStore(indexName)) {
      throw new IllegalArgumentException(
          "[" + indexName + "] is not a valid index name for a feature store");
    }
  ...
  /**
   * Returns if this index name is a possible index store The index must be {@link #DEFAULT_STORE}
   * or starts with {@link #STORE_PREFIX}
   *
  ...
  public <E extends StorableElement> E getAndParse(String name, Class<E> eltClass, String type)
      throws IOException {
  ...
  public static <E extends StorableElement> E parse(
      Class<E> eltClass, String type, byte[] bytes, int offset, int length) throws IOException {
    try (XContentParser parser =
        XContentFactory.xContent(bytes).createParser(XContentParserConfiguration.EMPTY, bytes)) {
      return parse(eltClass, type, parser);
    }
  }

  public static <E extends StorableElement> E parse(
      Class<E> eltClass, String type, BytesReference bytesReference) throws IOException {
    BytesRef ref = bytesReference.toBytesRef();
    try (XContentParser parser =
        XContentFactory.xContent(ref.bytes, ref.offset, ref.length)
            .createParser(XContentParserConfiguration.EMPTY, ref.bytes, ref.offset, ref.length)) {
      return parse(eltClass, type, parser);
    }
  }

  public static <E extends StorableElement> E parse(
      Class<E> eltClass, String type, XContentParser parser) throws IOException {
    ...
    if (!elt.type().equals(type)) {
      throw new IllegalArgumentException(
          "Expected an element of type [" + type + "] but got [" + elt.type() + "].");
    }
  ...
  /**
   * Validate the feature store name Must not bear an ambiguous name such as
   * feature/featureset/model and be a valid indexName
   *
  ...
  public static void validateFeatureStoreName(String storeName) {
    if (INVALID_NAMES.matcher(storeName).matches()) {
      throw new IllegalArgumentException(
          "A featurestore name cannot be based on the words [feature], [featureset] and [model]");
    }
    MetadataCreateIndexService.validateIndexOrAliasName(
        storeName,
        (name, error) ->
            new IllegalArgumentException("Invalid feature store name [" + name + "]: " + error));
  }
}
