...
public class LoggingFetchSubPhase implements FetchSubPhase {
  ...
  public FetchSubPhaseProcessor getProcessor(FetchContext context) throws IOException {
    LoggingSearchExtBuilder ext =
        (LoggingSearchExtBuilder) context.getSearchExt(LoggingSearchExtBuilder.NAME);
    if (ext == null) {
      return null;
    }
    ...
    // Delegate creation of the loggers until we know the hits checking for
    ...
    CheckedSupplier<Tuple<Weight, List<HitLogConsumer>>, IOException> weigthtAndLogSpecsSupplier =
        () -> {
          ...
          ext.logSpecsStream()
              .filter((l) -> l.getRescoreIndex() != null)
              .forEach(
                  (l) -> {
          ...
          Weight w =
              context
                  .searcher()
                  .rewrite(builder.build())
                  .createWeight(context.searcher(), ScoreMode.COMPLETE, 1.0F);
          return new Tuple<>(w, loggers);
        };

    return new LoggingFetchSubPhaseProcessor(
        Suppliers.memoizeCheckedSupplier(weigthtAndLogSpecsSupplier));
  }

  private Tuple<RankerQuery, HitLogConsumer> extractQuery(
      LoggingSearchExtBuilder.LogSpec logSpec, Map<String, Query> namedQueries) {
                ...
                () ->
                    new IllegalArgumentException(
                        "Query named ["
                            ...
                            + ((q instanceof BoostQuery)
                                ? ((BoostQuery) q).getQuery().getClass().getSimpleName()
                                : q.getClass().getSimpleName())
                            + "] found")));
  }

  private Tuple<RankerQuery, HitLogConsumer> extractRescore(
      LoggingSearchExtBuilder.LogSpec logSpec, List<RescoreContext> contexts) {
  ...
  private Tuple<RankerQuery, HitLogConsumer> toLogger(
      LoggingSearchExtBuilder.LogSpec logSpec, RankerQuery query) {
    HitLogConsumer consumer =
        new HitLogConsumer(logSpec.getLoggerName(), query.featureSet(), logSpec.isMissingAsZero());
  ...
  static class LoggingFetchSubPhaseProcessor implements FetchSubPhaseProcessor {
    ...
    LoggingFetchSubPhaseProcessor(
        CheckedSupplier<Tuple<Weight, List<HitLogConsumer>>, IOException> loggersSupplier) {
  ...
  static class HitLogConsumer implements LogLtrRanker.LogConsumer {
     ...
     *
     * <p>The Map is created on first access.
     */
    @Override
    ...
    DocumentField newLogField() {
      List<Object> logList =
          Collections.singletonList(new HashMap<String, List<Map<String, Object>>>());
