...
/**
 * Created by doug on 12/30/16. The ranklib models are treated like scripts in that they can be run
 * inline, cached, or stored in files However, we don't use the script query because we need to
 * execute many underlying queries.
 *
 * <p>So this code acts as a hook for deserializing Ranklib models from ranklib XML and as a
 * convenient means for caching those deserialized model
 */
public class RankLibScriptEngine implements ScriptEngine {
  ...
  public <T> T compile(
    ...
    RankLibModelContainer.Factory retFactory =
        () -> {
          LtrRanker ltrRanker =
              factory.getParser(RanklibModelParser.TYPE).parse(null, scriptSource);
