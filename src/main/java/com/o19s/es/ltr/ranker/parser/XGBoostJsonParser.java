...
public class XGBoostJsonParser implements LtrRankerParser {
  ...
  public NaiveAdditiveDecisionTree parse(FeatureSet set, String model) {
    ...
    try (XContentParser parser =
        JsonXContent.jsonXContent.createParser(XContentParserConfiguration.EMPTY, model)) {
  ...
  private static class XGBoostDefinition {
    ...
    static {
      ...
      PARSER.declareObjectArray(
          XGBoostDefinition::setSplitParserStates,
          SplitParserState::parse,
    ...
    public static XGBoostDefinition parse(XContentParser parser, FeatureSet set)
        throws IOException {
      XGBoostDefinition definition;
      XContentParser.Token startToken = parser.nextToken();

      // The model definition can either be an array of tree definitions, or an object containing
      // the
      // tree definitions in the 'splits' field. Using an object allows for specification of
      // additional
      // parameters.
      if (startToken == XContentParser.Token.START_OBJECT) {
        try {
          ...
          throw new ParsingException(
              parser.getTokenLocation(), "Unable to parse XGBoost object", e);
        }
        if (definition.splitParserStates == null) {
          throw new ParsingException(
              parser.getTokenLocation(), "XGBoost model missing required field [splits]");
        }
      ...
      if (definition.splitParserStates.size() == 0) {
        throw new ParsingException(
            parser.getTokenLocation(), "XGBoost model must define at lease one tree");
      }
      return definition;
    }
     ...
     *
     * <p>Depending on the objective, the model prediction may require normalization. Currently only
     * untransformed (noop) and logistic (sigmoid) types are implemented. See <a
     * href="https://xgboost.readthedocs.io/en/latest/parameter.html#learning-task-parameters">task
     * params</a>
     *
      ...
      switch (objectiveName) {
        ...
        default:
          throw new IllegalArgumentException(
              "Objective [" + objectiveName + "] is not a valid XGBoost objective");
      }
    }
  ...
  private static class SplitParserState {
    ...
    static {
      ...
      PARSER.declareObjectArray(
          SplitParserState::setChildren, SplitParserState::parse, new ParseField("children"));
    }
    ...
    public static SplitParserState parse(XContentParser parser, FeatureSet set) {
      ...
      if (split.isSplit()) {
        if (!split.splitHasAllFields()) {
          throw new ParsingException(
              parser.getTokenLocation(), "This split does not have all the required fields");
        }
        if (!split.splitHasValidChildren()) {
          throw new ParsingException(
              parser.getTokenLocation(),
              "Split structure is invalid, yes, no and/or"
                  + " missing branches does not point to the proper children.");
        }
        if (!set.hasFeature(split.split)) {
          throw new ParsingException(
              parser.getTokenLocation(), "Unknown feature [" + split.split + "]");
        }
      } else if (!split.leafHasAllFields()) {
        throw new ParsingException(
            parser.getTokenLocation(), "This leaf does not have all the required fields");
      }
      return split;
    }
    ...
    boolean splitHasValidChildren() {
      return children.size() == 2
          && leftNodeId.equals(children.get(0).nodeId)
          && rightNodeId.equals(children.get(1).nodeId);
    }
