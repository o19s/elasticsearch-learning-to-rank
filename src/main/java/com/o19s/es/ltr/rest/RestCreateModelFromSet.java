...
public class RestCreateModelFromSet extends FeatureStoreBaseRestHandler {
  ...
  protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client)
    ...
    if (expectedVersion != null) {
      builder.withVersion(
          store, request.param("name"), expectedVersion, state.model.name, state.model.model);
    } else {
    ...
    return (channel) ->
        builder.execute(
            ActionListener.wrap(
                response ->
                    new RestToXContentListener<CreateModelFromSetResponse>(
                            channel,
  ...
  private static class ParserState {
    ...
    private static final ObjectParser<ParserState, Void> PARSER =
        new ObjectParser<>("create_model_from_set", ParserState::new);

    static {
      PARSER.declareObject(
          ParserState::setModel, Model.MODEL_PARSER::apply, new ParseField("model"));
      PARSER.declareObject(
          ParserState::setValidation,
    ...
    private static class Model {
      ...
      private static final ObjectParser<Model, Void> MODEL_PARSER =
          new ObjectParser<>("model", Model::new);

      static {
        MODEL_PARSER.declareString(Model::setName, new ParseField("name"));
        MODEL_PARSER.declareObject(
            Model::setModel, StoredLtrModel.LtrModelDefinition::parse, new ParseField("model"));
      }
