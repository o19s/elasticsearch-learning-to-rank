...
public class TermStatQueryBuilder extends AbstractQueryBuilder<TermStatQueryBuilder>
  ...
  public static TermStatQueryBuilder fromXContent(XContentParser parser) throws IOException {
    ...
    if (builder.terms == null) {
      throw new ParsingException(
          parser.getTokenLocation(), "Field [" + TERMS_NAME + "] is mandatory");
    }

    if (builder.expr == null) {
      throw new ParsingException(
          parser.getTokenLocation(), "Field [" + EXPR_NAME + "] is mandatory");
    }
  ...
  protected int doHashCode() {
    return Objects.hash(
        expr, aggr, pos_aggr, Arrays.hashCode(terms), Arrays.hashCode(fields), analyzerName);
  }
