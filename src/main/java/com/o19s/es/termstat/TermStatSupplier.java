...
public class TermStatSupplier extends AbstractMap<String, ArrayList<Float>> {
  private final List<String> ACCEPTED_KEYS =
      Arrays.asList(new String[] {"df", "idf", "tf", "ttf", "tp"});
  ...
  /**
   * Returns {@code true} if this map contains a mapping for the specified stat type;
   *
   * @param statType Stat type to retrieve from the supplier
   * @return {@code true} if this map contains a mapping for the specified stat type
  ...
  /**
   * Returns the score to which the specified featureName is mapped, or {@code null} if this map
   * contains no mapping for the featureName.
   *
   * @param statType Stat type to retrieve from the supplier
   * @return the score to which the specified stat type is mapped, or {@code null} if this map
  ...
  /**
   * Strictly speaking the only methods of this {@code Map} needed are containsKey and get. The
   * remaining methods help fix issues like - deserialization of FEATURE_VECTOR parameter as a Map
   * object - keeps editors like intellij happy when debugging e.g. providing introspection into Map
   * object.
   */
  @Override
