...
public class CustomMustacheFactory extends DefaultMustacheFactory {
    ...
    protected static String extractVariableName(String fn, Mustache mustache, TemplateContext tc) {
      ...
      if (codes == null || codes.length != 1) {
        throw new MustacheException(
            "Mustache function [" + fn + "] must contain one and only one identifier");
      }
      ...
      try (StringWriter capture = new StringWriter()) {
        ...
        throw new MustacheException(
            "Exception while parsing mustache function [" + fn + "] at line " + tc.line(), e);
      }
    }
  }
  ...
  static class ToJsonCode extends CustomCode {
    ...
    ToJsonCode(TemplateContext tc, DefaultMustacheFactory df, Mustache mustache, String variable) {
      ...
      if (!CODE.equalsIgnoreCase(variable)) {
        throw new MustacheException(
            "Mismatch function code [" + CODE + "] cannot be applied to [" + variable + "]");
      }
    }
  ...
  static class CustomJoinerCode extends JoinerCode {
    ...
    CustomJoinerCode(
        TemplateContext tc, DefaultMustacheFactory df, Mustache mustache, String variable) {
  ...
  /**
   * This function encodes a string using the {@link URLEncoder#encode(String, String)} method with
   * the UTF-8 charset.
   */
      ...
      if (getCodes() != null) {
        for (Code code : getCodes()) {
          try (StringWriter capture = new StringWriter()) {
            ...
            throw new MustacheException(
                "Exception while parsing mustache function at line " + tc.line(), e);
          }
        }
      }
      return writer;
