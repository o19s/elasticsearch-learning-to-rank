    ...
    if (isCollectionScoped()) {
      ...
      for (Term term : terms) {
        ...
        if (ctx != null && ctx.docFreq() > 0) {
          ...
          idf_stats.add(
              sim.idf(tStats.docFreq(), searcher.collectionStatistics(term.field()).docCount()));
      ...
      */
      ...
      return new ConstantScoreWeight(ExplorerQuery.this, constantScore) {
        ...
        public Scorer scorer(LeafReaderContext context) throws IOException {
          return new ConstantScoreScorer(
              this, constantScore, scoreMode, DocIdSetIterator.all(context.reader().maxDoc()));
        }
      ...
      // FIXME: completely refactor this class and stop accepting a random query but a list of terms
      // directly
      // rewriting at this point is wrong, additionally we certainly build the TermContext twice for
      // every terms
      ...
      return new ExplorerQuery.ExplorerWeight(
          this, searcher.createWeight(q, scoreMode, boost), type);
    }
  ...
  private BooleanClause makeBooleanClause(Term term, String type) throws IllegalArgumentException {
    if (type.endsWith("_raw_tf")) {
      return new BooleanClause(
          new PostingsExplorerQuery(term, PostingsExplorerQuery.Type.TF),
          BooleanClause.Occur.SHOULD);
    } else if (type.endsWith("_raw_tp")) {
      return new BooleanClause(
          new PostingsExplorerQuery(term, PostingsExplorerQuery.Type.TP),
          BooleanClause.Occur.SHOULD);
    }
    throw new IllegalArgumentException("Unknown ExplorerQuery type [" + type + "]");
  }
